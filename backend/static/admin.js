...
// 1. PORTEIRO: Verifica se o usuário tem permissão antes de carregar qualquer coisa
...
// Executa a verificação assim que carregar o script
verificarAcessoAdmin();

// Variável global para gerenciar qual ticket estamos respondendo
                    ...
                    <tbody class="text-white">
                        ${data.map(aud => `
                            <tr class="border-b border-gray-800">
                                <td class="p-3 text-sm">${aud.email_usuario}</td>
                                <td class="p-3 text-sm">${aud.nome_arquivo}</td>
                                <td class="p-3 text-sm">${new Date(aud.created_at).toLocaleDateString()}</td>
                                <td class="p-3"><button onclick="baixarPdf('${aud.id}')" class="text-blue-400 hover:text-blue-300">Download</button></td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
...
async function carregarAdminTickets() {
    ...
    try {
        ...
        container.innerHTML = `
            <h2 class="text-2xl font-bold mb-6">Tickets de Suporte</h2>
            <div class="space-y-4">
                ${data.map(t => `
                    <div class="bg-gray-800 p-4 rounded-xl border border-gray-700 flex justify-between items-center">
                        <div>
                            <p class="font-bold">${t.assunto}</p>
                            <p class="text-sm text-gray-400">${t.texto}</p>
                        </div>
                        <button onclick="abrirModalTicket('${t.id}')" class="bg-[#991b1b] px-4 py-2 rounded-lg text-sm font-bold">Responder</button>
                    </div>
                `).join('')}
            </div>
        `;
...
// Adicione isso no seu static/admin.js
async function carregarAdminTickets() {
    const container = document.getElementById('admin-container');
            ...
            <div class="space-y-4">
                ${tickets.map(t => `
                    <div class="bg-gray-900 p-4 rounded-lg border border-gray-800">
                        ...
                        ${t.resposta ? `
                            <div class="bg-gray-800 p-3 rounded text-sm text-blue-300 mb-4">
                                <strong>Resposta:</strong> ${t.resposta}
                            </div>
                        ` : ''}
                        
                        ${t.status === 'aberto' ? `
                            <button onclick="abrirModalTicket('${t.id}')" class="text-xs bg-[#991b1b] text-white px-3 py-1 rounded hover:bg-[#7f1d1d]">
                                Responder
                            </button>
                        ` : ''}
                    </div>
